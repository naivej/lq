# lq - A CLI Tool for Editing LyX Files

`lq` is a standalone CLI tool designed to parse, query, and mutate LyX (`.lyx`) documents using a lossless Virtual DOM. It allows users to target document elements using CSS-like selectors without breaking the file formatting expected by the LyX WYSIWYM GUI.

### Highlight
- Easy **cross-references and citations**.
- **Tracked change** support.
- Designed for AI agents using skills.

### Limitation
- We do not know lyx's syntax. This tool is built by analysing all official templates (lyx -> file -> New from template...) and the author's personal lyx files.
...
2. **Test Blast Radius**: Run `lq read <file> <selector>`. 
   - *Blast Radius* refers to the number of nodes a selector matches. Because commands like `set` and `delete` apply to *all* matched nodes, an overly broad selector (e.g., `layout[Standard]`) could wipe out the entire document! Always use `read` to verify your selector targets exactly what you intend.
   - *Warning for `insert`*: If your selector matches 5 nodes, `insert` will duplicate the payload 5 times (once relative to each matched node).
3. **Execute Mutation**: Run `lq insert`, `lq set`, or `lq delete`. If you make a mistake, use `git restore` to revert the file.
...
`lq` exposes a JSON API designed for machine consumption.
...
- **LyX Compatibility**: developed and verified against **LyX 2.4 and LyX 2.5**. 
