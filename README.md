...
- **Windows LyXServer confirmations are unreliable**: LyX's Windows pipe server (`Server.cpp`) only flushes its reply buffer to client connections when its loop is woken by a new event, so a command's *response* can be lost even though the command executed. This is a LyX server behavior — verified that no client read strategy fixes it. `lq` therefore treats a command as dispatched once it is written to the pipe (LyX will execute it), and a lost confirmation only downgrades the outcome: `--refresh reload` warns that the buffer may not have been reloaded, and `--refresh save-reload` proceeds with a warning that the save was not confirmed (it was almost certainly applied), aborting only when LyX is genuinely unreachable or reports a save error. Repeated rapid connections degrade the server; a fresh LyX restart restores it.
