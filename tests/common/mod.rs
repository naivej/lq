...
/// LyX binary for headless export. `LYX_BINARY` / `LYX_PATH`, then layouts-dir
/// walk, then `%LOCALAPPDATA%\Programs\LyX *\bin\LyX.exe`.
pub fn find_lyx_binary() -> Option<PathBuf> {
    ...
    let layouts = lq::get_default_layouts_dir();
    if layouts.is_dir() {
