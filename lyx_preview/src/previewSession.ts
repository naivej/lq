...
/** Next step appended to MALFORMED_JSON and CONTRACT banners (DL036). */
export const LIVE_MISMATCH_HINT =
  " This lq does not match LyX Preview — set lyx-preview.lqPath to download the latest, or rebuild the local binary.";

function malformedJson(message: string): never {
  throw new AdapterError("MALFORMED_JSON", `${message}${LIVE_MISMATCH_HINT}`);
}

function contract(message: string): never {
  throw new AdapterError("CONTRACT", `${message}${LIVE_MISMATCH_HINT}`);
}
...
export function parseLiveStdout(stdout: string): LiveRender {
  ...
  try {
    ...
    malformedJson("lq returned invalid JSON.");
  }
  if (parsed === null || typeof parsed !== "object") {
    malformedJson("lq returned a non-object JSON payload.");
  }
  ...
  for (const entry of obj.outline) {
    ...
    if (
      ...
      contract("outline entry needs level/number/text/id.");
    }
  }
  if (obj.navigate === null || typeof obj.navigate !== "object") {
    contract("lq preview omitted navigate.");
  }
  ...
  for (const key of ["figures", "tables", "equations", "labels", "listings", "algorithms"]) {
    if (!Array.isArray(nav[key])) {
      contract(`navigate.${key} must be an array.`);
    }
  }
  if (!Array.isArray(obj.changes)) {
    contract("lq preview omitted changes.");
  }
  for (const entry of obj.changes) {
    if (entry === null || typeof entry !== "object") {
      contract("change entries must be objects.");
    }
    const e = entry as Record<string, unknown>;
    if (
      ...
      contract("change entry needs ordinal/type/author/ts/anchorId/snippet.");
    }
  }
  if (!Array.isArray(obj.tokens)) {
    contract("lq preview omitted tokens.");
  }
  const seenTokenIds = new Set<string>();
  for (const entry of obj.tokens) {
    if (entry === null || typeof entry !== "object") {
      contract("each token must be an object.");
    }
    const t = entry as Record<string, unknown>;
    if (typeof t.id !== "string" || t.id.length === 0) {
      contract("token.id must be a non-empty string.");
    }
    if (seenTokenIds.has(t.id)) {
      contract(`token.id '${t.id}' is not unique.`);
    }
    seenTokenIds.add(t.id);
    if (t.bundle === null || typeof t.bundle !== "object") {
      contract("token.bundle must be an object.");
    }
    const b = t.bundle as Record<string, unknown>;
    if (typeof b.selector !== "string" || b.selector.length === 0) {
      contract("token.bundle.selector must be a non-empty string.");
    }
    if ("file" in b && b.file !== undefined && b.file !== null) {
      if (typeof b.file !== "string" || b.file.length === 0) {
        contract("token.bundle.file must be a non-empty string when present.");
      }
      if (typeof b.diskHash !== "string" || b.diskHash.length === 0) {
        contract("token.bundle.diskHash must be a non-empty string when file is set.");
      }
    }
    if ("via" in b && b.via !== undefined && b.via !== null) {
      if (typeof b.via !== "object") {
        contract("token.bundle.via must be an object when present.");
      }
      const v = b.via as Record<string, unknown>;
      if (typeof v.file !== "string" || v.file.length === 0) {
        contract("token.bundle.via.file must be a non-empty string.");
      }
      if (typeof v.selector !== "string" || v.selector.length === 0) {
        contract("token.bundle.via.selector must be a non-empty string.");
      }
    }
