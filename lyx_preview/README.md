...
A VS Code companion extension for [`lq`](https://github.com/naivej/lq) — a standalone CLI to parse, query, and mutate LyX documents.

This extension renders a read-only preview of saved `.lyx` files directly inside a VS Code webview, combining visual feedback, outline navigation, and AI-assisted workflows.
...
- **Status Bar Inspection:** Displays the active `lq` selector, and tracked-change author/timestamp metadata in the VS Code status bar.
...
3. Add `lqsel.json` to `.gitignore`.
4. Run `npx skills add naivej/lq` to install the `use-lq` skill for AI agent workflows.
