...
**W5 — Extra or fewer rows/columns** after look exists: `add-row` / `add-column` (optional `--data`); `delete-row` / `delete-column` (`--index` required). The last remaining row or column is refused. Replay the table (`at` / `inset[Tabular]`) to revert a tracked row or column; replay a cell layout to peel `--data` / cell `set` only.
